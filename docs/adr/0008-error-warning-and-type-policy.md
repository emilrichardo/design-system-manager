...
### `$type` efectivo (herencia)
`tipo propio del token → tipo heredado del grupo ancestro más cercano → error si no existe`
(`dtcg-type-undeterminable`). Se informa **tipo declarado**, **tipo efectivo** y **origen**
(`own | alias | group | none`). `$extensions` **no** legitima tipos desconocidos.
